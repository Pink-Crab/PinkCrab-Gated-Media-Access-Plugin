...
/**
 * Builds the four sections we ship and hands the collection to `gatedmedia_account_sections`.
 *
 * That filter is the only place third-party code adds UI, and it adds sections rather than registering services.
 *
 * Resolved once per request, so the nav, the router and the renderer all read the same collection and a section cannot appear in one and be missing from another.
 */
class Section_Registry {
	...
	public function all(): Section_Collection {
		 ...
		 *
		 * Add one with `Section_Collection::add()`, which returns a new collection to return rather than mutating in place.
		 *
		 * @param Section_Collection $sections The sections so far.
		 */
		$filtered = apply_filters( 'gatedmedia_account_sections', $this->defaults() );

		// A filter returning the wrong thing would take the whole account area down, and a broken third-party filter should not cost the user their downloads.
	...
	/**
	 * The four built-in sections, each its own implementation of `Account_Section`.
	 *
	 * Classes rather than `Section` value objects, deliberately: the interface is the contract third parties write against, so ours implementing it first is what proves it sufficient. A page with behaviour of its own then has somewhere to put it.
	 *
	 * Positions are spaced in tens so a third party can land between any two without renumbering ours.
	 */
