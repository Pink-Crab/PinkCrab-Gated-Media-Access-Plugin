 ...
 *
 * The component specs need a page holding every component on ordinary page furniture, and it has to be built here rather than by hand, or the specs pass on one machine and nowhere else.
 *
 * Skipped when WP_BASE_URL points somewhere that is not wp-env: there is no safe way to create posts on a site we do not own.
 */

const { spawnSync } = require( 'node:child_process' );
const path = require( 'node:path' );

// wp-env mounts the project under the directory name, which in CI is the run id, not the slug.
 ...
 *
 * `wp eval-file` wraps the file, which breaks `declare( strict_types )`, so the fixture is included instead.
 *
 * spawnSync rather than execFileSync keeps stderr: wp can report a failed include there and still exit 0, which left this throwing an error with nothing in it.
 *
...
module.exports = async () => {
	...
	for ( const file of FIXTURES ) {
		...
		// A product's URL carries a uuid minted when the fixture ran, so the specs are handed it rather than hardcoding one.
